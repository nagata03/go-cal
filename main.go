...
func main() {
	...
	wday := time.Date(t.Year(), t.Month(), 1, 0, 0, 0, 0, time.Local).Weekday()  // 今月初日の曜日を取得する
	days := time.Date(t.Year(), t.Month()+1, 1, 0, 0, 0, 0, time.Local).AddDate(0, 0, -1).Day()  // 今月の最終日を取得する
